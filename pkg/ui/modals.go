...
func (vm *ViewManager) QuitModal(_ *gocui.Gui, _ *gocui.View) error {
	AddModalView(vm, "quit the program?", func(_ *gocui.Gui, _ *gocui.View) error {
...
func (vm *ViewManager) ReloadModal(_ *gocui.Gui, _ *gocui.View) error {
	AddModalView(vm, "reload dictionary?", func(_ *gocui.Gui, v *gocui.View) error {
		...
		*vm.State = *s
		...
		for name := range vm.Views {
			...
			if viewsV, err := vm.g.View(name); err != nil {
				return err
			} else {
